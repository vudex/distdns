...
class MonitorThread(powergslb.system.AbstractThread):
    ...
    _check_params_types = {
        ...
        'code': {'type': str, 'url': str, 'host': str, 'interval': int, 'timeout': int, 'fall': int, 'rise': int}
    }
    ...
    def _start_check_threads(self):
        ...
        for check in self._checks:
            ...
            check_thread = CheckThread(check['monitor_json'], check['id'], self.cache, name=thread_name)
            check_thread.start()
    ...
    def task(self):
        try:
          with Cache(**powergslb.system.get_config().items('cache')) as self.cache:
            self._update_checks()
